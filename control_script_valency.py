...
this script is used to define variables for a specific trial and then run it
...
trial_description = '2_valency_testv7_'
...
nrep = 5#50 # number of reptitions for each sensitivity case
n = 20#0 # number of agents
...
nwpks = ['m'] # network parameter keys (must be empty if more than one network kinds, or must match with network kind; use empty list for no network parameters)

# define model/analysis parameters not to include in the sensitivity analysis
tmax=10#00 # number of time steps to iterate over (make longer to get correct diffusion time)
...
stratifieds = [False] # True to impose weighted influence based on follower count
nwp0s = [0.005,0.01,0.05,0.1,0.2] # vals to correspond with 1st key in nwpks, e.g. if nwpks=['k','p'], you'd list the sensitivity cases of p here
nwp1s = [None]*len(nwp0s) # " " 2nd " "
...
labels = ['$m$ = '+str(val) for val in nwp0s]
